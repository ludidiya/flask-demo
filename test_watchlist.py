...
class WatchlistTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        app.config.update(
            ...
            # SQLALCHEMY_DATABASE_URI 设为 'sqlite:///:memory:'
            # 这会使用 SQLite 内存型数据库，不会干扰开发时使用的数据库文件
    ...
    def tearDown(self):
        ...
        db.drop_all()  # 删除数据库表，测试时的程序状态和真实的程序运行状态不同，所以需要调用 db.session.remove() 来确保数据库会话被清除。
